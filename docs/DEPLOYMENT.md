...
- `app/config.js` — generated by `deploy-production.sh` using `SPA_DOMAIN` and `API_DOMAIN` from the server's `.env`
- `config/env/production.json` — generated by `generate-formio-config.sh` using the server's `.env`
...
- Regenerates `config/env/production.json` from the server's `.env` (via `generate-formio-config.sh`)
